document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", function (event) {
        ...
        if (profileImage) {
            ...
            reader.onloadend = function () {
                ...
                generateResume(formData, imageUrl);
            };
            reader.readAsDataURL(profileImage);
        } else {
            // Generate resume without image
    ...
    function generateResume(data, imageUrl = "") {
        resumeContent.innerHTML = `
            ${imageUrl ? `<img src="${imageUrl}" alt="Profile Image" style="width: 150px; height: 150px; border-radius: 75px; display: block; margin: 0 auto;">` : ""}
            <h3>${data.name}</h3>
